...
void ILevelBase::Configure(Config* _config, Screen* _screen)
{
	...
	const Vec2 worldOffseFactor = m_config->GetValue<Vec2>("world", "offsetFactor");
	const Vec2 worldScale = m_config->GetValue<float>("world", "scale");

	float sW, sH;
	m_screen->GetSize(&sW, &sH);

	const Vec2 worldCenter = Vec2{ sW * worldOffseFactor.x,sH * worldOffseFactor.y };
